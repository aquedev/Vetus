...
namespace CssCombiner.NAntTask
{
    ...
    public class CssCombiner : Task
    {
        ...
        [TaskAttribute("assemblyVersion", Required = true)]
        ...
        private string AppendBuildVersionToImagesUrls(Match m)
        {
            ...
            return m.Groups[1] + url + m.Groups[3];
        }
        ...
        private string AppendBuildVersionToImagesUrls(string file)
        {
            var r = new Regex(@"(url[\s]*\([\s]*)(.+)(\).*)", RegexOptions.IgnoreCase);

            var result = r.Replace(file, new MatchEvaluator(AppendBuildVersionToImagesUrls));
